...
namespace MemberMicroservice.Controllers
{
    ...
    public class MembersController : ControllerBase
    {
        ...
        [HttpPost]
        [Route("updateMember")]
        public async Task<ActionResult<string>> UpdateMember([FromQuery] int memberID, [FromQuery] string memberName, [FromQuery] string memberAddress1, [FromQuery] string memberAddress2, [FromQuery] string memberCity, [FromQuery] int memberPhone, string username, string password, string role)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    _log4net.Info("UpdateMember Accesed");
                    try
                    {
                        var selectedMember = MemberData.members.Where(m => m.MemberID == memberID).First();
                        ...
                        MemberData.members.Remove(selectedMember);
                        MemberData.members.Add(newMember);
